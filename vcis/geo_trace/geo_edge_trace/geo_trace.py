...
class GeoTrace:
    ...
    def merge_stay_points(self, data, verbose=True):
        ...
        if verbose:
            display("\n data view before main computation \n", data)

        merged_df = pd.DataFrame(columns=data.columns)
        stay_points = []
        current_stay_point = None

        for index, row in data.iterrows():
            if row['Label'] == 'Stay Point':
                if current_stay_point is None:
                    current_stay_point = {'size': row['Size'], 'duration': row['Duration'], 
                                        'Latitude': row['Latitude'], 'Longitude': row['Longitude'], 
                                        'start_time': row['Start_Time'], 'end_time': row['End_Time']}
                    
                else:
                    # Check haversine distance between consecutive stay points
                    distance = row['Distance']
                    if distance <= 20 / 1000:  # Converted 20 meters to kilometers
                        # Merge stay points
                        current_stay_point['size'] += row['Size']
                        current_stay_point['duration'] += row['Duration']
                        current_stay_point['Latitude'] = (current_stay_point['Latitude'] + row['Latitude']) / 2
                        current_stay_point['Longitude'] = (current_stay_point['Longitude'] + row['Longitude']) / 2
                        current_stay_point['end_time'] = row['End_Time']
                    else:
                        # Save current merged stay point
                ...
                # Fill points, directly append to the merged dataframe
                merged_df = merged_df.append(row, ignore_index=True)

        # Add the last stay point if exists
        if current_stay_point is not None:
            stay_points.append(current_stay_point)

        # Append merged stay points to the merged dataframe
        for stay_point in stay_points:
            merged_df = merged_df.append({
                ...
                'Duration': stay_point['duration'],
                'Size': stay_point['size'], 
                'Label': 'Stay Point'}, ignore_index=True)
            
        if verbose:
            display("\n merged_df view after main computation \n", merged_df)
        
        merged_df.sort_values(by='Start_Time', inplace=True)
    ...
    def evaluate_gap_in_time(self, data):
        ...
        print("\nNumber of fill points before filtering based on time: ", len(fill_points))
        ...
        # print number of fill points
        print("\nNumber of fill points after filtering based on time: ", len(fill_points))
        ...
        print("\nNumber of fill points after filtering based on distance: ", len(fill_points))
    ...
    def preprocess_data_for_christophers_trace(self, data, graph):
        ...
        print("Adjusted data: ", data.columns)
    ...
    def trace_single_device(self, data):
        ...
        # Step 4: Evaluate Movement Type & Initialize Graphs Per Segment
        ...
        for time_period, [graph, data] in data_segments.items():
            ...
            # # Step 5: Perform Christopher's Trace Algorithm
        ...
        # Step 6: Merge results for each device, if possible
