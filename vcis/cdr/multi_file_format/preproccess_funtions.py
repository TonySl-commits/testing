...
class CDR_Preproccess_fucntions():
    ...
    def leb_number_transformation(self,passed_phone_no, is_called ):
        """
        ...
        if passed_phone_no.startswith(tuple(self.country_codes_list)):
            matched_code = next((code for code in self.country_codes_list if passed_phone_no.startswith(code)), None)
            if matched_code:
    ...
    def add_location_columns(self, passed_dataframe, passed_bts_table):
        ...
        joined_df_1 = passed_dataframe.join(passed_bts_table, 
                                    passed_dataframe["cgi_id"] == passed_bts_table[self.properties.cgi_id],
                                    "left")
        selected_columns = [
            passed_dataframe["*"],
            joined_df_1[self.properties.location_latitude].alias("location_latitude"),  # Replace "column1" with the actual column names
            joined_df_1[self.properties.location_longitude].alias("location_longitude"),
            joined_df_1[self.properties.location_azimuth].alias("location_azimuth")
        ]
